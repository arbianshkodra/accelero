...
- **Broad compose field support**: `entrypoint`, `working_dir`, `user`, `hostname`, `domainname`, `dns`, `dns_search`, `extra_hosts`, `cap_add`, `cap_drop`, `privileged`, `tmpfs`, `shm_size`, `init`, `expose`, `stop_grace_period`, `stop_signal`, `pull_policy` (always/missing/never; build rejected), `logging` (driver + options), long-form `ports` with per-binding `host_ip` and `protocol`, top-level `volumes:` (named volumes with scoped naming `accelero_<stack>_<name>`, idempotent create, `external: true` verification, never auto-delete). See docs/usage-overview.md#supported-compose-fields for the full table.
