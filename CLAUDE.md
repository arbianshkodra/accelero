...
- **Notifications**: notable deploy/drift lifecycle events pushed to a generic JSON webhook and/or Slack incoming webhook (`NOTIFY_WEBHOOK_URL` / `NOTIFY_SLACK_WEBHOOK_URL`). Best-effort, async, never blocks a deploy. Implemented as an `audit.Recorder` wrapper (`internal/notify`) so no deployer/reconciler changes were needed.
...
- `NOTIFY_WEBHOOK_URL` / `NOTIFY_SLACK_WEBHOOK_URL`: optional notification destinations for notable deploy/drift lifecycle events. `NOTIFY_WEBHOOK_URL` receives the full Event JSON (`type`/`stack`/`outcome`/`message`/`fields`/`time`); `NOTIFY_SLACK_WEBHOOK_URL` receives a Slack `{"text": ...}` payload. Events notified: deploy start/complete/failed/rolled_back, drift detected, drift auto_deployed (read/secret-CRUD/other audit ops are deliberately not notified). Delivery is best-effort + async (one goroutine per sink, 10s timeout, no retries) — a failing sink never blocks or fails a deploy. Implemented in `internal/notify` by **wrapping the shared `audit.Recorder`** (`notify.New` + `notify.WrapRecorder` in `cmd/main.go`), so the deployer/reconciler are untouched — they already record these audit events. Both unset = no-op (`Notifier.Enabled()==false`, base recorder returned unchanged).
...
- `notify/notify_test.go`: `New` disabled with no URLs / enabled with either; webhook sink gets the full Event JSON and Slack sink gets a `{"text":...}` payload (both against `httptest.Server`); `WrapRecorder` notifies only on deploy/drift ops while persisting *all* entries through to the base recorder; `WrapRecorder` returns the base unchanged when the notifier is disabled; `eventFromAudit` emits the right emoji + backtick-quoted stack for each notable op and returns `ok=false` for others
