...
- `ACCELERO_ENCRYPTION_KEY_VAULT`: the master key **wrapped by HashiCorp Vault Transit** (`vault:v1:…`). Accelero POSTs it to `<VAULT_ADDR>/v1/<mount>/decrypt/<key>` once at startup and keeps the unwrapped 32 bytes in memory only — the master key is never at rest in plaintext, and rotating the Transit KEK never requires re-encrypting DB fields. Requires `VAULT_ADDR`, `VAULT_TOKEN`, `ACCELERO_VAULT_TRANSIT_KEY`; optional `ACCELERO_VAULT_TRANSIT_MOUNT` (default `transit`) and `ACCELERO_VAULT_NAMESPACE` (Enterprise). **Fails closed** — missing required vars, unreachable Vault, or a token that can't decrypt is a fatal startup error, never a silent fallback to plaintext. Implemented in `internal/secrets/vault.go` with plain `net/http` (no new dep); the unwrap is bounded by a 30s context and the response read is capped at 1 MiB. Mutually exclusive with the two vars above (exclusivity is judged on *selector presence* so "two sources set" is reported ahead of any completeness error). Wrap a key with: `vault write -field=ciphertext transit/encrypt/accelero plaintext="$(openssl rand -base64 32)"`.
...
- `GET /api/v1/volumes/{name}/browse` — list files in a managed volume or (with `?download=true`) stream one file's contents. `?host=<name|id>` selects the daemon (omitted/`default` = `DOCKER_SOCK`). Read-only; backed by an ephemeral busybox helper container mounted at `/volume:ro`. 10MB download cap, 10k-entry listing cap, `..` paths rejected, volumes.browse/read audited.
- `POST /api/v1/volumes/{name}/files?path=<p>&mode=<oct>[&host=<name|id>]` — write file bytes into a managed volume (emergency patch affordance). Gated behind `ALLOW_VOLUME_WRITES=true`; 403 otherwise. 10MB body cap, parent dirs auto-created, `..` rejected. Audited as `volume.write`.
...
- `secrets/vault_test.go`: Transit unwrap against a fake Vault (`httptest`) — correct decrypt path/token/namespace/ciphertext sent, custom mount honoured, Vault's `errors[]` surfaced *without* echoing the wrapped key, empty-plaintext and unreachable-Vault errors; env wiring (Vault source produces a working cipher, missing required vars name all three, sources mutually exclusive, and no Vault vars leaves the inline path untouched)
...
- `handler/multihost_test.go` (volume section): `?host=` routes browse/write to that host's browser and leaves the default one untouched; the managed-check runs on the *requested* host so a default-host-only volume 404s when a remote host is named (the same-name trap); omitted and explicit `default` both use the default host; unknown host 400; remote host without wiring 503; the host lands in the volume audit metadata
