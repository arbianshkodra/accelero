...
- **Multi-host**: the `DOCKER_SOCK` daemon is the implicit **default host**; additional daemons are registered via `/api/v1/hosts` (`unix://`/`tcp://`, optional mutual TLS — `tls_key` encrypted at rest, never serialised) and selected per stack with `stacks.host_id` (empty = default, so existing stacks are unaffected). `internal/dockerhost.Manager.ClientFor(hostID)` builds+caches one client per host. **Host-aware today:** deploy (`Deployer.forHost` — a shallow Deployer copy with `cli` swapped, so the ~20 `d.cli` call sites are untouched; lock state is behind a pointer to stay shared) and drift/reconcile/`/preview` (`Reconciler.dockerFor` → threaded into `checkDrift`/`listStackContainers`). **Still default-host-only:** container introspection (logs/stats/exec/events/restart), root browsers (`/images`,`/volumes`,`/networks`), and `cleanupLoop`. Registration pings before persisting; deleting an in-use host is 409.
