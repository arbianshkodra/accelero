...
- **Notifications**: notable deploy/drift/approval lifecycle events pushed to a generic JSON webhook, Slack, Discord, and/or Microsoft Teams (`NOTIFY_WEBHOOK_URL` / `NOTIFY_SLACK_WEBHOOK_URL` / `NOTIFY_DISCORD_WEBHOOK_URL` / `NOTIFY_TEAMS_WEBHOOK_URL`). Each sink fires independently, best-effort, async, never blocks a deploy. Implemented as an `audit.Recorder` wrapper (`internal/notify`, `notify.New(notify.Config{...})`) so no deployer/reconciler changes were needed.
...
- `NOTIFY_WEBHOOK_URL` / `NOTIFY_SLACK_WEBHOOK_URL` / `NOTIFY_DISCORD_WEBHOOK_URL` / `NOTIFY_TEAMS_WEBHOOK_URL`: optional notification destinations for notable deploy/drift lifecycle events. `NOTIFY_WEBHOOK_URL` receives the full Event JSON (`type`/`stack`/`outcome`/`message`/`fields`/`time`); `NOTIFY_SLACK_WEBHOOK_URL` receives a Slack `{"text": ...}` payload; `NOTIFY_DISCORD_WEBHOOK_URL` a Discord `{"content": ...}`; `NOTIFY_TEAMS_WEBHOOK_URL` a Microsoft Teams `MessageCard`. Each sink fires independently. `notify.New` takes a `notify.Config` struct (one field per sink). Events notified: deploy start/complete/failed/rolled_back, drift detected, drift auto_deployed (read/secret-CRUD/other audit ops are deliberately not notified). Delivery is best-effort + async (one goroutine per sink, 10s timeout, no retries) — a failing sink never blocks or fails a deploy. Implemented in `internal/notify` by **wrapping the shared `audit.Recorder`** (`notify.New` + `notify.WrapRecorder` in `cmd/main.go`), so the deployer/reconciler are untouched — they already record these audit events. Both unset = no-op (`Notifier.Enabled()==false`, base recorder returned unchanged).
